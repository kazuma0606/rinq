...
fn main() {
    ...
    let prices = vec![
        100.0_f64, 102.0, 101.0, 105.0, 107.0,
    ...
    println!("\nEMA(α=0.3): {:?}", ema.iter().map(|x| format!("{x:.2}")).collect::<Vec<_>>());

    // SMA (window=3)
    let sma = QueryBuilder::from(prices.clone()).simple_moving_average(3);
    println!("SMA(3):     {:?}", sma.iter().map(|x| format!("{x:.2}")).collect::<Vec<_>>());

    // WMA (window=3)
    let wma = QueryBuilder::from(prices.clone()).weighted_moving_average(3);
    println!("WMA(3):     {:?}", wma.iter().map(|x| format!("{x:.2}")).collect::<Vec<_>>());

    // Rate of Change
    let roc = QueryBuilder::from(prices.clone()).rate_of_change(1);
    println!("ROC(1)%:    {:?}", roc.iter().map(|x| format!("{x:.2}")).collect::<Vec<_>>());
